...
/*   Updated: 2018/01/13 20:53:40 by shural           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			ft_termlstadd(t_list **list, char *str)
{
	...
	if (str)
	{
		...
		while (i < 19)
		{
			if (x + 5 * (g_ts[i][1] - '0') + g_ts[i][2] - '0' < 21 &&
				x + 5 * (g_ts[i][3] - '0') + g_ts[i][4] - '0' < 21 &&
				x + 5 * (g_ts[i][5] - '0') + g_ts[i][6] - '0' < 21 &&
				str[x + 5 * (g_ts[i][1] - '0') + g_ts[i][2] - '0'] == '#' &&
				str[x + 5 * (g_ts[i][3] - '0') + g_ts[i][4] - '0'] == '#' &&
				str[x + 5 * (g_ts[i][5] - '0') + g_ts[i][6] - '0'] == '#')
			{
				ft_lstaddback(list, ft_lstnew(g_ts[i], 11));
				return (1);
			}
			i++;
		}
